...
type Plan = { id: string; name: string; price_cents: number; billing_interval: string | null; credits_allocated: number };
...
export default function Account() {
  ...
  return (
    <AccountShell onLogout={() => void logout()} email={session.user?.email}>
      ...
      {checkoutStatus === "success" && <Notice tone={hasCurrentSubscription ? "success" : "neutral"}>{hasCurrentSubscription ? "Subscription active. Your Seller plan is ready." : "Confirming your subscription. Refresh in a moment if your account has not updated yet."}</Notice>}
...
function SubscriptionBadge({ status, canceled }: { status: string; canceled: boolean }) { const label = canceled ? "Ending" : status === "past_due" || status === "unpaid" ? "Payment issue" : status === "incomplete" ? "Setup incomplete" : status === "trialing" ? "Trial" : "Active"; const color = label === "Active" || label === "Trial" ? "bg-emerald-50 text-emerald-700" : label === "Payment issue" ? "bg-red-50 text-red-700" : "bg-slate-100 text-slate-700"; return <span className={`rounded-md px-2.5 py-1 text-xs font-semibold ${color}`}>{label}</span>; }